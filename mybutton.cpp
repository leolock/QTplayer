...
StopButton::StopButton(QWidget *parent):
    QPushButton(parent)
{
    QPixmap pixmap(":/images/stop_1.png");

    resize(pixmap.size());
//    resize(100,100);
    /* 设置按钮的有效区域 */
    setMask(QBitmap(pixmap.mask()));
    setStyleSheet("QPushButton{border-image: url(:/images/stop_1.png);}");
}

void StopButton::enterEvent(QEvent *)
{
    setStyleSheet("QPushButton{border-image: url(:/images/stop_2.png);}");
}

void StopButton::leaveEvent(QEvent *)
{
    setStyleSheet("QPushButton{border-image: url(:/images/stop_1.png);}");
}
//快进按钮
...
//    resize(100,100);
    /* 设置按钮的有效区域 */
    setMask(QBitmap(pixmap.mask()));
    setStyleSheet("QPushButton{border-image: url(:/images/sound.png);}");
}
//静音按钮
...
//    resize(100,100);
    /* 设置按钮的有效区域 */
    setMask(QBitmap(pixmap.mask()));
    setStyleSheet("QPushButton{border-image: url(:/images/mute.png);}");
}
